...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, "Neural Networks - final")
        #the coordinates of the agent's food and tunnel will be stored in these
        self.input = []
        self.target = []
        self.hiddenNodes = [0]*15
        ...
        self.trainingSet = []
        self.initHiddenWeights = []
        self.initOutputWeights = []
        self.layer1 = []
        self.layer2 = []
    ...
    def getMove(self, currentState):
        ...
        if self.training:
            self.evaluateState(currentState)#append to the list of states, then call back propigation
    ...
    def forwardPropigation(self, currentState):
        self.input = self.mapping(currentState) #get the input set
        self.allInputs.append(self.input)
        self.target = self.evaluateState(currentState) #set the target value with this state

        #combine the inputs and their weights
        finalInput = np.array(self.allInputs) #should be of size 28, we should create lots of these
        #let's associate each input with one target
        inputLayerSize = 28
        hiddenLayerSize = 15
        outputLayerSize = 1

        self.initHiddenWeights = np.random.uniform(low=-1.0, high=1.0, size=(inputLayerSize, hiddenLayerSize))
        self.initOutputWeights = np.random.uniform(low=-1.0, high=1.0, size=(hiddenLayerSize, outputLayerSize))

        #forward propogation, first through hidden layer, then to output layer
        print self.input, self.target
        self.layer1 = self.nonlin(np.dot(finalInput, self.initHiddenWeights))
        self.layer2 = self.nonlin(np.dot(self.layer1,self.initOutputWeights))

        self.learnedScores.append(self.layer2) #this should be what we pass as
        output = self.layer2
        return output
    ...
    #
    # added in bias nodes and a learning rate to help adjust values over time.
    # When we call backPropigate we should pass in the evaluate state value!
    #
    def backPropigate(self, targets):

        #pairArray = zip(self.input, self.target) #associate our input with the target
        finalTargets = np.array(targets)

        #generate this from
        finalInput = np.array(self.allInputs)

        # how much did we miss the target value?
        l2_error = finalTargets - self.learnedScores
        ...
        print self.learnedScores.shape
        l2_delta = l2_error*self.nonlin(self.learnedScores,deriv=True)

        # how much did each l1 value contribute to the l2 error (according to the weights)?
        l1_error = l2_delta.dot(self.initOutputWeights.T)

        # were we really sure? if so, don't change too much.
        l1_delta = l1_error * self.nonlin(self.layer1,deriv=True)

        print self.initOutputWeights.shape, self.layer1.T.shape, l2_delta.shape
        self.initOutputWeights += (self.layer1.T.dot(l2_delta))*self.learningRate
        self.initHiddenWeights += (finalInput.T.dot(l1_delta))*self.learningRate

        self.learnedOutputWeights = self.initOutputWeights
        self.learnedHiddenWeights = self.initHiddenWeights
        self.learnedScores = self.layer2
        #for the number of states we've seen, add to our target array?
        return l2_delta, l1_delta
    ...
    def evaluateState(self, currentState):
        ...
        # if self.training:
        #     self.trainingSet.append(stateScore)
        #     return stateScore
        return stateScore
    ##
    # bestNodeScore
    ...
    def recursiveMove(self, currentState, currentDepth):
        ...
        for i, move in enumerate(moveList):
            ...
            if self.training:
                ...
                nodeList[i]['score'] = self.forwardPropigation(nodeList[i]['nextState'])
        ...
        if (currentDepth >= self.maxDepth):
            if self.training:
                ...
                return self.forwardPropigation(currentState)
    ...
    def registerWin(self, hasWon):
        #if self.training:
            #shuffle our collection of states

            #for every state in the states we've collected, use backPropigation on the collection
        if hasWon:
            print "we won!!"
        return hasWon
